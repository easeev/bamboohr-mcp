...
- **29 tools** covering the full BambooHR API surface
...
```json
{
  "mcpServers": {
    "bamboohr": {
      "command": "node",
      "args": ["/path/to/bamboohr-mcp/dist/index.js"],
      "env": {
...
```bash
git clone https://github.com/easeev/bamboohr-mcp.git
cd bamboohr-mcp
npm install
npm run build
```
...
- **get-applications** — List candidates with filtering by job, status,
  search, rating
- **get-application-details** — Full candidate profile, Q&A, attachments,
  education
- **get-application-comments** — Get all comments/notes on an application
- **get-applicant-statuses** — List available status IDs for
  update-application-status
- **get-attachment-url** — Get authenticated download URL and curl command for
