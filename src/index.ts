...
registerRecruitingTools(server); // 7: get-jobs, get-applications, get-application-details, update-application-status, add-application-comment, get-applicant-statuses, download-attachment

async function main() {
  const transport = new StdioServerTransport();
  await server.connect(transport);
  console.error('cl-bamboohr-mcp server started');
}
